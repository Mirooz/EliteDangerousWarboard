...
/**
 * Détecte le quadrilatère de calibration tracé en <strong>rose / magenta</strong> (HSV) sur une capture
 * {@code leftpanel} et retourne les quatre coins en <strong>pixels</strong> image (ordre {@link PanelCorners}).
 */
public final class RoseLeftPanelPositionDetector {

    /**
     * Teinte magenta / rose néon (OpenCV H 0–180). Plage élargie autour des tracés type bordure HUD rose.
     */
    private static final Scalar HSV_ROSE_LOW = new Scalar(128, 40, 70);
    private static final Scalar HSV_ROSE_HIGH = new Scalar(178, 255, 255);

    private RoseLeftPanelPositionDetector() {
    }
    ...
    public static Optional<PanelCorners> detectPixelCorners(Mat bgr) {
        ...
        try {
            ...
            Core.inRange(hsv, HSV_ROSE_LOW, HSV_ROSE_HIGH, mask);
